...
static NSString *const identify = @"ClientMineProductTabCell";
@interface ClientMineProductCViewModel()<UITableViewDataSource,UITableViewDelegate>
...
@end
@implementation ClientMineProductCViewModel
...
- (instancetype)init {
    ...
    [self.homeViewModel.favoriteListCommand.executionSignals.switchToLatest subscribeNext:^(id  _Nullable x) {
            MMJF_Log(@"%@",x);
...
- (void)bindViewToViewModel:(UITableView *)view {
    ...
    self.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            NSDictionary *dic = @{@"dict":@{@"type":@"1"},@"page":[NSString stringWithFormat:@"%ld",weakSelf.pageCurrent]};
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ClientMineProductTabCell *cell = [tableView dequeueReusableCellWithIdentifier:identify];
    if (!cell) {
        cell = [[[NSBundle mainBundle]loadNibNamed:@"ClientMineProductTabCell" owner:self options:nil]lastObject];
    }
    NSDictionary *dic = self.dataSource[indexPath.section];
    ClientMineProductModel *model = [ClientMineProductModel yy_modelWithJSON:dic];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        NSDictionary *dic = @{@"id":[NSString stringWithFormat:@"%@",model.object_id],@"type":@"1",@"action":@"cancel"};

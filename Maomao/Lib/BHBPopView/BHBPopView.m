...
@property (nonatomic, strong)ShopWindowsView *shareView;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        ShopWindowsView *shareView = [[[NSBundle mainBundle]loadNibNamed:@"ShopWindowsView" owner:self options:nil] lastObject];
        shareView.frame = self.bounds;
        __weak typeof(self)weakSelf = self;
        [[shareView.shutDownBut rac_signalForControlEvents:UIControlEventTouchUpInside]subscribeNext:^(__kindof UIControl * _Nullable x) {
            [[BHBPlaySoundTool sharedPlaySoundTool] playWithSoundName:@"close"];
            [weakSelf.bottomBar btnResetPosition];
            [weakSelf.bottomBar fadeOutWithTime:.25];
            [weakSelf hideItems];
            [weakSelf hide];
        }];
        BHBItem *item = [[BHBItem alloc]init];
        [[shareView.createBut rac_signalForControlEvents:UIControlEventTouchUpInside]subscribeNext:^(__kindof UIControl * _Nullable x) {
            [[BHBPlaySoundTool sharedPlaySoundTool] playWithSoundName:@"close"];
            [weakSelf.bottomBar btnResetPosition];
            [weakSelf.bottomBar fadeOutWithTime:.25];
            [weakSelf hideItems];
...
//        logo.image = [UIImage imageNamed:@"chuang-jian-dian-pu"];
...
//        };
//        bar.closeClick = ^{
//            [[BHBPlaySoundTool sharedPlaySoundTool] playWithSoundName:@"cha"];
//            [weakSelf hideItems];
//            [weakSelf hide];
//        };
//        [self addSubview:bar];
//        self.bottomBar = bar;
//        BHBCenterView * centerView = [[BHBCenterView alloc]initWithFrame:CGRectMake(0, self.frame.size.height * 0.2, self.frame.size.width, self.frame.size.height * 0.4)];

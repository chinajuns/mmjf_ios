...
- (void)layoutBtnsWith:(NSArray *)items isMore:(BOOL)isMore{
    ...
    BHBItem * item;
    NSInteger k = items.count / 2 + items.count % 2;
    for (int i = 0; i < items.count; i ++) {
        ...
        CGFloat x = (i % k) * self.frame.size.width / k;
        CGFloat y = (i / k) * self.frame.size.height / 2.0;
        if (isMore) {
        ...
        CGFloat width = self.frame.size.width / k;

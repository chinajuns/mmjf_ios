...
#pragma mark - 操作标签方法
// 添加多个标签
- (void)addTags:(NSArray *)tagStrs selectedStrs:(NSArray *)selectedStrs
{
    ...
    for (NSString *tagStr in tagStrs) {
        BOOL isOK = NO;
        for (NSString *selectedStr in selectedStrs) {
            if ([tagStr isEqualToString:selectedStr]) {
                isOK = YES;
            }
        }
        [self addTag:tagStr isOk:isOK];
    }
}
// 添加标签
- (void)addTag:(NSString *)tagStr isOk:(BOOL)isOk
{
    ...
    if (isOk == YES) {
        [tagButton setTitleColor:_tagSelectedColor forState:UIControlStateNormal];
        [tagButton setBackgroundColor:_tagSelectedBackgroundColor];
    ...
    // 更新自己的高度
    if (_isFitTagListH) {
        MMJF_Log(@"sssss%f,%f",self.biggestH,self.tagListH);
        if (self.biggestH == 0) {
        ...
        if (self.tagListH < self.biggestH) {
            CGRect frame = self.frame;
            frame.size.height = self.tagListH;
            [UIView animateWithDuration:0.25 animations:^{
                self.frame = frame;
            ...
            CGRect frame = self.frame;
            frame.size.height = self.biggestH;
            [UIView animateWithDuration:0.25 animations:^{
                self.frame = frame;
            }];
        }
...
- (void)setupTagButtonCustomFrame:(UIButton *)tagButton preButton:(UIButton *)preButton extreMargin:(BOOL)extreMargin
{
    ...
    CGFloat titleH;
    if (_tagLabelH == 0) {
        titleH = [tagButton.titleLabel.text sizeWithFont:_tagFont].height;
    }else{
        titleH = _tagLabelH;
    }

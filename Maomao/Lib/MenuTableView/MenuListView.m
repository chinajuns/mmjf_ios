...
- (UILabel *)titleLabel
{
    if (!_titleLabel) {
        ...
        CGFloat titleLabelH = self.height - 2*10.f;
        _titleLabel.frame = CGRectMake(15.f, 10.f, self.width - 100.f, titleLabelH);
...
- (UIImageView *)imgView
{
    if (!_imgView) {
        ...
        CGFloat imgViewWH = self.height - 2*10.f;
        _imgView.contentMode = UIViewContentModeCenter;
        _imgView.frame = CGRectMake(MMJF_WIDTH - 15.f - imgViewWH, 10.f, imgViewWH, imgViewWH);
    }
    return _imgView;
}
...
//        self.imgView.image = [UIImage imageNamed:@"yi-xuan"];
//        // NSLog(@"选中了");
//    }else {
...
- (void)subViewsWithTitles:(NSArray *)titles defImage:(UIImage *)defImage selImage:(UIImage *)selImage
{
    ...
    // 上边的分割线
//    CGRect topLineFrame = CGRectMake(0, 1.f, MMJF_WIDTH, 0.5f);
//    UIView *topLine = [[UIView alloc] initWithFrame:topLineFrame];
//    topLine.backgroundColor = kLineCor;
//    [self addSubview:topLine];
    
    // 下边的分割线
    CGRect bottomLineFrame = CGRectMake(0, self.height - 0.5f, MMJF_WIDTH, 0.5f);
    ...
    for (int i=0; i<count; i++) {
        // 创建按钮
        CGFloat buttonW = (self.width - (count-1)*kLineW)/count;
        ...
        if (i < count-1) {
            ...
            CGFloat lineY = (self.height-kLineH)/2;
...
- (UIView *)shadow
{
    if (!_shadow) {
        ...
        _shadow.frame = CGRectMake(0, y, MMJF_WIDTH, MMJF_HEIGHT);
...
- (UITableView *)lTableView
{
    if (!_lTableView) {
        ...
        CGRect frame = CGRectMake(0.f, y + self.height, self.width, 0.f);
...
- (CGFloat)maxListHeightWithModel:(NSArray *)dataSource
{
    ...
    oriHeight > MMJF_HEIGHT/4*2 ? (height = MMJF_HEIGHT/4*2) : (height = kRowH*count);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.title = [dic objectForKey:_titleKey];
    cell.imgView.image = nil;
    if ([[dic objectForKey:_titleKey] isEqualToString:_currenTitle]) {
        cell.imgView.image = [UIImage imageNamed:@"yi-xuan"];
    }
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    _button.isSeled = NO;
    NSDictionary *dic = self.dataSource[indexPath.row];
    _button.title = [dic objectForKey:_titleKey];
    
    [_button resetStatus:_button];
    
    // 实现回调方法
    if (self.clickListView) {
        self.clickListView(_button.tag, indexPath.row, self.dataSource[indexPath.row]);
    }
}

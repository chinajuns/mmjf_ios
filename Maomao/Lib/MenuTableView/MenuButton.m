...
- (UILabel *)titLabel
{
    if (!_titLabel) {
        ...
        _titLabel.textColor = [UIColor colorWithHexString:@"#4d4d4d"];
        UIFont *font = [UIFont systemFontOfSize:kFontSize];
        _titLabel.font = font;
        CGSize size = [_title sizeWithFont:font maxSize:CGSizeMake(self.width, self.height)];
        _titLabel.center = CGPointMake(self.width/2 - kTriangleWH/2, self.height/2);
...
- (UIImageView *)imgView
{
    if (!_imgView) {
        ...
        _imgView.frame = CGRectMake(CGRectGetMaxX(self.titLabel.frame), (self.height-kTriangleWH)/2, kTriangleWH, kTriangleWH);
    }
    return _imgView;
}
...
- (void)menuButtonClicked
{
    ...
    if (_selected) {
        self.titLabel.textColor = MMJF_COLOR_Yellow;
        self.imgView.image = _selImg;
    }
    else {
        self.titLabel.textColor = [UIColor colorWithHexString:@"#4d4d4d"];
...
{
    if (!isSeled) {
        _selectedBtn.titLabel.textColor = [UIColor colorWithHexString:@"#4d4d4d"];
...
- (void)setTitle:(NSString *)title
{
    ...
    CGSize size = [title sizeWithFont:font maxSize:CGSizeMake(self.width
                                                              , self.height)];
    _titLabel.center = CGPointMake(self.width/2 - kTriangleWH/2, self.height/2);
    _titLabel.bounds = CGRectMake(0, 0, size.width, size.height);
    _imgView.frame = CGRectMake(CGRectGetMaxX(self.titLabel.frame), (self.height-kTriangleWH)/2, kTriangleWH, kTriangleWH);
...
- (void)resetStatus:(MenuButton *)button
{
    ...
    button.titLabel.textColor = [UIColor colorWithHexString:@"#4d4d4d"];

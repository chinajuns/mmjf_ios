...
-(void)viewDidLoad{
    ...
    self.searchBar = [[UISearchBar alloc]initWithFrame:CGRectMake(0, 22, MMJF_WIDTH - 80, 44.0f)];
    ...
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, MMJF_WIDTH, 66.0f)];
    [view addSubview:self.searchBar];
    UIButton *but = [UIButton buttonWithType:UIButtonTypeCustom];
    but.frame = CGRectMake(self.searchBar.width, 22, 60, 40);
    [but setTitle:@"取 消  " forState:UIControlStateNormal];
    [but setTitleColor:[UIColor colorWithHexString:@"#4d4d4d"] forState:UIControlStateNormal];
    but.titleLabel.font = [UIFont fontWithName:@"PingFang SC" size:16];
    ...
    [[but rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(cancel:)]) {
            [weakSelf.delegate cancel:weakSelf];
        }
    ...
    [self.tableView setSectionIndexColor:[UIColor colorWithHexString:@"#ffd200"]];
...
//        return cell;
        ...
        if (indexPath.section == 0) {
            GYZCity *city = [self.localCityData objectAtIndex:indexPath.row];
            [cell.textLabel setText:[NSString stringWithFormat:@"当前定位城市:%@",city.cityName]];
            
        }else{
            GYZCity *city = [self.hotCityData objectAtIndex:indexPath.row];
            [cell.textLabel setText:city.cityName];
        }
        return cell;
    }

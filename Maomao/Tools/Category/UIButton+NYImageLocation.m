...
- (void)setImageLocation:(YZImageLocation)location spacing:(CGFloat)spacing maxWith:(CGFloat)maxWith{
    CGFloat imageWith = self.imageView.image.size.width;
    CGFloat imageHeight = self.imageView.image.size.height;
    
    NSDictionary *dic = @{NSFontAttributeName:self.titleLabel.font};
    CGFloat titleWidth = [self.titleLabel.text sizeWithAttributes:dic].width;
    if (titleWidth > maxWith) {
        titleWidth = maxWith - 20;
    }
    CGFloat titleHeight = [self.titleLabel.text sizeWithAttributes:dic].height;
    //image中心移动的x距离
...
- (void)setImageLocation:(YZImageLocation)location spacing:(CGFloat)spacing offSet:(YZOffSetDirection)offSetDirection offSetVar:(CGFloat)offSetVar{
    CGFloat imageWith = self.imageView.image.size.width;
    CGFloat imageHeight = self.imageView.image.size.height;
    NSDictionary *dic= @{NSFontAttributeName:self.titleLabel.font};
    //title的宽度

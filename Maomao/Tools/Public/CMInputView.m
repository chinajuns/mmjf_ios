...
- (void)textDidChange
{
    NSString *str = [NSString disable_emoji:self.text];
    // 根据文字内容决定placeholderView是否隐藏
    self.placeholderView.hidden = self.text.length > 0;
    
    NSInteger height = ceilf([self sizeThatFits:CGSizeMake(self.bounds.size.width, MAXFLOAT)].height);
    if (self.text.length > _maxNumber) {
        self.text = [str substringToIndex:_maxNumber];
        return;
    ...
    if (_textH != height) { // 高度不一样，就改变了高度
        ...
        if (_textChangedBlock && self.scrollEnabled == NO) {
            _textChangedBlock(str,height);
            
            [self.superview layoutIfNeeded];
            self.placeholderView.frame = self.bounds;

        }else{
            _textChangedBlock(str,_maxTextH);
        }
    }else{
        _textChangedBlock(str,0);
    }
